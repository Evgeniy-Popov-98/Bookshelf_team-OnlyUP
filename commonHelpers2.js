...
`;async function b(){try{const t=l(n);if(!t||!t.length){o.innerHTML=c;return}let s="";for(const e of t){const i=await h(e.constID);s+=y(i,e)}o.innerHTML+=s}catch(t){console.error("Error fetching book data:",t)}}function y(t,s){const e=t.description?t.description:"With our diverse range of titles, you're sure to find the perfect companion for cozy nights in. Treat yourself to the joy of reading and explore the endless possibilities that await within the pages of our books.";return console.log(e),`
<div class="container-block" id="${s.constID}">    
  ...
  `}o.addEventListener("click",function(t){const s=t.target;if(t.target.parentNode.nodeName==="BUTTON"||t.target.nodeName==="BUTTON"){const e=s.closest(".container-block"),i=e.getAttribute("id");let a=[];const p=l(n);for(const r of p)r.constID!==i&&a.push(r);g(n,a),e.remove(),o.querySelector(".container-block")||(o.innerHTML=c)}});b();
